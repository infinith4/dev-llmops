...
このデモでは、日本語テキストの Non-LLM 評価指標として `NonLLMStringSimilarity` と `RougeScore` を採用する。`BleuScore` もあわせて出力するが、日本語では参考値として扱う。
...
このデモでは、日本語テキストの評価指標として `ragas-non_llm_string_similarity` と `ragas-rouge_score` を使用し、`ragas-bleu_score` は「日本語文に対しては参考値」として扱う。

- `ragas-non_llm_string_similarity`: 主評価指標。文字列の近さを確認する
- `ragas-rouge_score`: 主評価指標。日本語でも BLEU より解釈しやすい
- `ragas-bleu_score`: 日本語では 0.000 でも直ちに不正解を意味しない

したがって、デモ画面や説明では「日本語では BLEU が低く出やすい」ことを理解した上で、`NonLLMStringSimilarity` と `RougeScore` を評価結果として確認する。
